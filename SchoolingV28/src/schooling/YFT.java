//The Yellowfin Tuna Object - contains all movement rules that Yellowfin tuna will follow.  
...
class YFT implements Steppable {
	...
	static List<String> YFTlist = new ArrayList<String>();
	
	//Fish object with location, previous angle, random movement probability
	//distance from origin and time stopped
	YFT( Double2D loc , int pangle, int randomMovementProbability, double distOrigin, int stopped, double minTemp, double maxTemp, double currentTemp, int timeFAD, int idnum)
	{
		setLoc( loc );
		setPangle( pangle );
	...
	public void step(SimState state) 
	{
		...
		//prints out the coordinated of all the YFT at specified times
		timestep++;
		if (timestep == BET.timeCheck) {
			System.out.println("YFT" + "," + BET.timeCheck + "," + loc.x + "," + loc.y + "," + currentTemp + "," + timeFAD + "," + idnum);
			YFTlist.add("YFT" + "," + BET.timeCheck + "," + loc.x + "," + loc.y + "," + currentTemp + "," + timeFAD + "," + idnum);
		}
	
		if ( BET.timesList.contains(timestep) ) {
			id++;
			System.out.println("YFT" + "," + id + "," + loc.x + "," + loc.y + "," + currentTemp + "," + timeFAD + "," + idnum);
			YFTlist.add("YFT" + "," + id + "," + loc.x + "," + loc.y + "," + currentTemp + "," + timeFAD + "," + idnum);
			}
	}
	...
	private void move()
	{
	    ...
	    if (loc.x <= 2 | loc.x >= Ocean.width-2 | loc.y <= 2 | loc.y >= Ocean.height-2) {  //if touching an edge of the map
        	...
        	bestx = Ocean.spaceYFT.stx( loc.x + locIncrements.x );
    		besty = Ocean.spaceYFT.sty( loc.y + locIncrements.y );    	
    		setPangle( angle );
        	            ...
        	            for (int y=-1;y<2;y++)
        	                if (!(x==0 && y==0))
        	                    ...
        	                    if (bestx==START ||
        	                        ...
        	                        { bestx = Ocean.spaceYFT.stx(x + loc.x); besty = Ocean.spaceYFT.sty(y + loc.y); }
	...
	{
		this.loc = loc;
		Ocean.spaceYFT.setObjectLocation( this, loc );
	}
